...
def change_to_ff(byte_stream):
    splited = []
    for byte in byte_stream:
        temp = []
        four_one = byte & 15
        four_two = (byte >> 4) & 15
        temp.append(four_two)
        temp.append(four_one)
        splited.append(temp)
    return splited
...
def listen_linux(frame_rate=44100, interval=0.1):
    ...
    while True:
        ...
        if in_packet and match(dom, HANDSHAKE_END_HZ):
            print("HAND SHAKE END")
            byte_stream = extract_packet(packet)
            try:
                byte_stream = RSCodec(FEC_BYTES).decode(byte_stream)
                byte_stream = byte_stream.decode("utf-8")
                if "201502117" in byte_stream: 
                    real_data = byte_stream.replace("201502117", "")
                    byte_stream = RSCodec(FEC_BYTES).encode(real_data)
                    byte_stream = change_to_ff(byte_stream)
                    new_byte_stream = []
                    for byte in byte_stream:
                        new_byte_stream.append(byte[0]*STEP_HZ + START_HZ)
                        new_byte_stream.append(byte[1]*STEP_HZ + START_HZ)
                    new_byte_stream.insert(0, HANDSHAKE_START_HZ)
                    new_byte_stream.insert(0, HANDSHAKE_START_HZ)
                    new_byte_stream.append(HANDSHAKE_END_HZ)
                    new_byte_stream.append(HANDSHAKE_END_HZ)
                    ...
                    samples = np.ndarray([])
                    for freq in new_byte_stream:
                        samples = (np.sin(2*np.pi*np.arange(44100*0.5)*freq/44100)).astype(np.float32)
